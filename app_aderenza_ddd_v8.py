...
st.title("Analisi aderenza terapeutica (PDC) basata su DDD – Calcolo a intervalli")
...
if file_disp and file_ddd:
    ...
    if submitted:
        ...
        # Funzione per calcolo PDC a intervalli per singolo paziente
        def calcola_pdc_paziente(s):
            ...
            ev = ev.append({date_col: end, "giorni_coperti": 0}, ignore_index=True).sort_values(date_col)
            ...
            pdc = covered_days / int(periodo)
            return pd.Series({
                "PDC": min(pdc, 1.0),
                "Copertura_giorni": covered_days,
                "Durata": int(periodo),
        ...
        riepilogo.columns = ["N_pazienti", "N_aderenti", "PDC_medio", "PDC_std"]
        ...
        # GRAFICO PER ATC
        st.subheader("📈 Aderenza media per ATC")
        fig = px.bar(riepilogo, x="ATC_principale", y="PDC_medio",
                     text="PDC_medio",
                     title="PDC medio per categoria terapeutica",
                     labels={"PDC_medio": "PDC medio"})
