# aderenza_intervalli_excel_v3.py
...
if disp_file and ddd_file:
    ...
    if dedup:
        ...
        disp = (
            g.rename("__coperti_tmp__")   # evita collisioni
             .reset_index()
    ...
    # ---------------- RIEPILOGO STRATIFICATO (fix collisione reset_index) ----------------
    # Se group_by_col == col_ther (o coincide con una key), evitiamo la collisione usando un nome temporaneo
    s = (
    ...
    strat_map = (
        s.rename("__strat_tmp__")   # evita conflitto di nomi con le chiavi dell'indice
         .reset_index()             # ora DataFrame senza collisioni
         .rename(columns={"__strat_tmp__": group_by_col})
    )
