...
st.set_page_config(page_title="Aderenza - Intervalli", layout="wide")
...
if disp_file and ddd_file:
    ...
    # suggerisce un campo 'Principio'/'ATC' se presente
    sug = next((c for c in disp.columns if "Principio" in c or "ATC" in c or "terap" in c.lower()), disp.columns[0])
    col_ther = st.selectbox("Colonna terapia/gruppo (DISP, es. Principio Attivo)", disp.columns, index=list(disp.columns).index(sug))
    ...
    group_by_col = st.selectbox("Stratifica e riepiloga per:", group_candidate_cols,
    ...
    # Dedup opzionale [cf, ther, __KEY__, date]
    if dedup:
        ...
        disp = (disp.groupby(group_keys, dropna=False)["__giorni_coperti_disp__"].sum().reset_index())
    ...
    for val, gdf in out.groupby(group_by_col, dropna=False):
        fig_box.add_trace(go.Box(
            y=gdf["ADH_anno"], name=str(val),
            boxpoints="all", jitter=0.4, pointpos=0, boxmean="sd"
        ))
