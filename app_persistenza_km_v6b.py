...
if file_disp:
    ...
    with st.form("setup"):
        ...
        with col2:
            ...
            default_cutoff = df[date_col].dropna().max()
    ...
    if submitted:
        ...
        if km_df.empty:
            st.warning("Nessun paziente eleggibile.")
        else:
            fig = go.Figure()
            ...
            st.download_button("💾 Scarica Excel risultati", buffer.getvalue(), "persistenza_km_results.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
else:
