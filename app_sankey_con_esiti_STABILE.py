...
st.title("Sankey terapeutico – esiti finali corretti")
...
if file:
    ...
    if invia:
        ...
        # Costruzione transizioni tra linee
        ...
        # Unione e preparazione nodi/etichette
        df_sankey = pd.concat(sankey_data, ignore_index=True)
        df_sankey = df_sankey.dropna()
        df_sankey.iloc[:, 0] = df_sankey.iloc[:, 0].astype(str)
        df_sankey.iloc[:, 1] = df_sankey.iloc[:, 1].astype(str)
        all_labels = pd.concat([df_sankey.iloc[:, 0], df_sankey.iloc[:, 1]]).unique().tolist()
        df_sankey["source"] = df_sankey.iloc[:, 0].apply(lambda x: all_labels.index(str(x)))
        df_sankey["target"] = df_sankey.iloc[:, 1].apply(lambda x: all_labels.index(str(x)))
        df_sankey["Count"] = df_sankey["Count"].astype(int)

        # Sankey definitivo
        fig = go.Figure(go.Sankey(
            node=dict(
                label=[str(x) for x in all_labels],
                pad=15,
                thickness=20,
        ...
        st.subheader("📈 Sankey terapeutico (versione stabile)")
        ...
        st.download_button("⬇️ Scarica risultati in Excel", data=buffer.getvalue(), file_name="sankey_esiti_stabile.xlsx")
