...
if disp_file and ddd_file:
    ...
    if miss > 0:
        st.warning(f"⚠️ {miss} righe senza DDD_standard_giornaliera → escluse dalla stima giorni coperti")
    ...
    # Dedup opzionale (PATCH robusta: usa __KEY__ solo se presente)
    if st.checkbox("Somma duplicati stesso giorno/paziente/terapia", value=True):
        group_keys = [col_cf, col_ther, col_date]
        if "__KEY__" in disp.columns:
            group_keys.insert(2, "__KEY__")  # [cf, ther, __KEY__, date]
        disp = (
            disp.groupby(group_keys, as_index=False)
    ...
    if method == "Intervalli (tipo Access)":
        for (cf, ther), g in disp.sort_values(col_date).groupby([col_cf, col_ther]):
            ...
            if adh_intervals:
                ADH_anno = min(sum(adh_intervals)/len(adh_intervals), 1.0)  # normalizza a 1
        ...
        for (cf, ther), g in disp.groupby([col_cf, col_ther]):
            ...
            pdc = min(giorni_coperti/period_days, 1.0)  # normalizza a 1
    ...
    if not res.empty:
        ...
        # --- export Excel: pazienti / media_ATC / totali ---
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine="openpyxl") as writer:
            ...
            pd.DataFrame({
                ...
                "Min":[vmin],
                "Max":[vmax],
                "Pazienti_aderenti_(>=0.8)":[(valori>=0.8).mean()]
