...
st.title("Sankey terapeutico – esiti e font migliorati")
...
if file:
    ...
    if invia:
        ...
        # Costruzione transizioni tra linee (escludendo identici)
        ...
        for i in range(1, max_linea):
            ...
            pivot = pivot[pivot[i] != pivot[i+1]]  # esclude loop identici
            if not pivot.empty:
                transizioni = pivot.groupby([i, i+1]).size().reset_index(name="Count")
                sankey_data.append(transizioni)

        # Transizione finale: ultima terapia → esito
        ...
        # Sankey Plot corretto (senza "snap")
        ...
        st.subheader("📈 Sankey terapeutico con esiti migliorati")
        ...
        st.download_button("⬇️ Scarica risultati in Excel", data=buffer.getvalue(), file_name="sankey_esiti_migliorato.xlsx")
