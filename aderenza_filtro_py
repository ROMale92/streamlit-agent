...
st.title("Analisi prescrittiva farmaci – PDC con DDD (pulita)")
...
if file and ddd_file:
    ...
    if submit:
        ...
        # ❗ Filtro: almeno 2 dispensazioni per il PDC
        multi_disp_ids = (
            df_linee.groupby(id_col)[date_col]
            .nunique()
            .reset_index(name="n_disp")
        )
        multi_disp_ids = multi_disp_ids[multi_disp_ids["n_disp"] >= 2][id_col]
        df_linee = df_linee[df_linee[id_col].isin(multi_disp_ids)]
        ...
        aderenza["PDC"] = aderenza["PDC"].clip(upper=1.0)  # ❗ Cap a 1.0
        ...
        st.download_button("Scarica risultati in Excel", data=excel_buffer.getvalue(), file_name="analisi_aderenza_ddd_filtrata.xlsx")
