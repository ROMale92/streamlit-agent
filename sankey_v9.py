...
def _pretty_label(s: str, maxlen: int = 28) -> str:
    """Etichette con a capo su '+' e prima di '(Linea N)', in Title Case."""
    s = str(s).strip()
...
with st.form("setup"):
    ...
    with c5:
        label_min_total = st.number_input("Mostra etichetta se traffico totale ≥", 0, 999, 30, 1)
    ...
    with c10:
        fig_height = st.number_input("Altezza grafico (px)", 400, 4000, 1100, 50)
...
# traffico per nodo (entrate + uscite) per ordinare e per mostrare le label
...
# posizione verticale: ordina nodi per traffico decrescente in ogni fase
y_pos = {}
...
# opacità link proporzionale al peso relativo
rel = (sankey_df["Count"] / tot_src).fillna(0).clip(0, 1)
gamma = 0.6  # più contrasto tra piccoli e grandi
...
# etichette: mostra solo sopra soglia
labels_pretty = [
