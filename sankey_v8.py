...
st.title("Analisi linee terapeutiche - Sankey (tutte le categorie)")
...
def _collapse_consecutive(df, id_col, cat_col):
    """Toglie ripetizioni consecutive della stessa categoria per ogni paziente."""
...
def _stage_from_label(label: str) -> int:
    """Ritorna la ‘fase’ (colonna) dal testo ‘(... Linea N)’; esiti a destra."""
    m = re.search(r"\(Linea\s+(\d+)\)$", str(label))
    return int(m.group(1)) if m else 10_000  # esiti

def _pretty_label(s: str, maxlen: int = 26) -> str:
    """Etichette compatte, con a-capo su ‘+’ e prima di ‘(Linea N)’, in Title Case."""
    s = str(s).strip()
    # a capo su '+' e prima di '(Linea N)'
    s = s.replace(" + ", " + ").replace(" (Linea", " (Linea")  # normalizza
    s = s.replace(" + ", "<br>+ ")
    s = s.replace(" (Linea", "<br>(Linea")
    plain = re.sub(r"<br>", " ", s).title()
    if len(plain) > maxlen:
        plain = plain[:maxlen] + "…"
    # rimetti i <br> nei punti desiderati
    plain = plain.replace(" + ", "<br>+ ").replace(" (Linea", "<br>(Linea")
    return plain
...
with st.form("setup"):
    ...
    with c2:
        ...
        cutoff_followup = st.date_input("📅 Cut-off follow-up (stato finale)", value=pd.Timestamp.today().date())
...
df[cat_col] = df[cat_col].astype(str).str.strip()  # niente upper fisso: lasciamo le maiuscole come in input
...
if sankey_df.empty:
    st.warning("Tutti i flussi sono sotto la soglia selezionata.")
    st.stop()

# ---------- layout nodi fisso per fase ----------
...
# posizione orizzontale per “Linea”, Esito a destra
...
# link grigi con opacità proporzionale al peso relativo
rel = (sankey_df["Count"] / tot_src).fillna(0).clip(0, 1)
alphas = (float(link_alpha_min) + (1.0 - float(link_alpha_min)) * rel).round(3)
link_colors = [f"rgba(100,100,100,{a})" for a in alphas]

# etichette compatte e “belle”
...
fig.update_layout(
    ...
    font=dict(family=font_family, size=int(font_size), color="#222"),
    plot_bgcolor="white",
    paper_bgcolor="white"
)
