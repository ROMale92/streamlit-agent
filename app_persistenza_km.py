...
st.title("Analisi persistenza terapeutica – Curva Kaplan–Meier (Plotly)")
...
if file_disp:
    ...
    if submitted:
        ...
        for strat, g in km_df.groupby(strat_col):
            ...
            # estendi fino al periodo
            if times[-1] < periodo:
                times.append(periodo)
                surv.append(s)

            fig.add_trace(go.Scatter(x=times, y=surv, mode="lines+markers", step="post", name=str(strat)))
